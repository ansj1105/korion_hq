 ...
 */
export function useLeaderSales(leaderCode?: string) {
  ...
  const accountInfo: InfoItem[] = [
    { label: t('hqLeaderSales.account.loginId'), value: pageData.account.loginId },
    { label: t('hqLeaderSales.account.password'), value: pageData.account.password, actionLabel: t('common.reset') },
    { label: t('hqLeaderSales.account.email'), value: pageData.account.email, actionLabel: t('common.change') },
    { label: t('hqLeaderSales.account.telegram'), value: pageData.account.telegram },
    { label: t('hqLeaderSales.account.phone'), value: pageData.account.phone },
    { label: t('hqLeaderSales.account.twitter'), value: pageData.account.twitter },
    { label: t('hqLeaderSales.account.appliedAt'), value: pageData.account.appliedAt, valueColor: 'var(--color-accent-green)' },
    { label: t('hqLeaderSales.account.approvedAt'), value: pageData.account.approvedAt, valueColor: 'var(--color-accent-green)' },
  ]

  // 2번째 줄은 본사 직접 계약 사유(1열) 다음 칸을 비우고 KORION WALLET 주소가 3열에 옴(Figma 실측 — 4열 그리드 중 2열은 빈칸)
  const basicInfo: InfoItem[] = [
    { label: t('hqLeaderSales.basic.name'), value: pageData.basic.name },
    { label: t('hqLeaderSales.basic.country'), value: pageData.basic.country },
    { label: t('hqLeaderSales.basic.region'), value: pageData.basic.region },
    { label: t('hqLeaderSales.basic.language'), value: pageData.basic.language },
    { label: t('hqLeaderSales.basic.directContractReason'), value: pageData.basic.directContractReason },
    { label: '', value: '' },
    { label: t('hqLeaderSales.basic.walletAddress'), value: pageData.basic.walletAddress },
  ]
  ...
  return {
    ...
    kpiBottom,
    logColumns,
    logRows: pageData.logRows as LeaderSalesLogRow[],
    settlement: pageData.settlement as { lastSettleDate: string; thisRequestAmount: string; status: string; rows: LeaderSettlementRow[] },
    settlementColumns,
    isLoading,
    error,
  }
