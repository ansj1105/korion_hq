 ...
 * ------------------------------------------------------------------
 * /api/hq/requests/partner-by-leader 응답만 사용한다. 실패 시 정적 샘플을 노출하지 않는다.
 */
export function useRequestsPartnerByLeader() {
  ...
  const columns: Column[] = [
    ...
    { key: 'status', label: t('hqRequestPartnerByLeader.col.status'), width: '0.8fr' },
  ...
  return {
    stats,
    columns,
    rows: pageData.rows as PartnerByLeaderRequestRow[],
    statusMeta,
